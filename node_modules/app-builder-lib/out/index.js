...
function build(options, packager = new (_packager().Packager)(options)) {
  ...
  const promise = packager.build().then((() => {
    var _ref = (0, _bluebirdLst().coroutine)(function* (buildResult) {
      ...
      if (afterAllArtifactBuild != null) {
        const newArtifacts = (0, _builderUtilRuntime().asArray)((yield Promise.resolve(afterAllArtifactBuild(buildResult))));

        if (newArtifacts.length === 0 || !publishManager.isPublish) {
          return buildResult.artifactPaths;
        }

        const publishConfigurations = yield publishManager.getGlobalPublishConfigurations();
